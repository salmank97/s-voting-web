...
describe('CandidateComponent', () => {
  ...
  it('should update the table when a Candidate is added', fakeAsync(() => {
    ...
    sinon.stub(component.serviceCandidate, 'addParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Candidate is updated', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.serviceCandidate, 'updateParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Candidate is deleted', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.serviceCandidate, 'deleteParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
